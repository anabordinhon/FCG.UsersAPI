...
Microsserviço responsável pelo gerenciamento de usuários do ecossistema **FIAP Cloud Games**. Este projeto implementa o cadastro de usuários e atua como **Producer**, publicando eventos de integração para outras APIs (como a NotificationsAPI) via RabbitMQ.

* * * * *

⚙️ **Configurações (Variáveis de Ambiente)**

Para o funcionamento correto, a API depende das seguintes configurações definidas no ambiente de orquestração (K8s):

-   **ConnectionStrings__DefaultConnection**: String de conexão com o banco SQL Server (Configurado via Secret).

-   **RabbitMq__Host**: Endereço/DNS do servidor RabbitMQ. Exemplo: `rabbitmq` (Configurado via ConfigMap).

-   **RabbitMq__Username**: Usuário para autenticação no Broker. Exemplo: `guest` (Configurado via ConfigMap).

-   **RabbitMq__Password**: Senha para autenticação no Broker (Dado Sensível configurado via Secret).

-   **Jwt__SecretKey**: Chave mestra para assinatura e validação de tokens JWT (Dado Sensível configurado via Secret).
...
-   Kubernetes habilitado nas configurações do Docker Desktop.
...
1.  **Acessar a API (Swagger)** A API estará exposta via LoadBalancer na porta 80: 👉 http://localhost/swagger

2.  **Acessar o RabbitMQ (Management)** Para visualizar as filas e conexões: 👉 http://localhost:15672 User: `guest` | Pass: `guest`
...
-   ✅ **Log de Sucesso**: Registra a persistência no banco. Mensagem: "Cadastro concluído/persistido". `UserId: {Guid}`, `CorrelationId: {Guid}`.

-   ✅ **Log de Erro**: Registra falhas de validação ou banco de dados. Mensagem: "Falha crítica no cadastro (Validação/DB)". `CorrelationId: {Guid}`.

-   ✅ **Log de Publicação**: Registra o envio do evento para o RabbitMQ. Mensagem: "UserCreatedEvent publicado". `EventId: {Guid}`, `CorrelationId: {Guid}`.

-   🚫 **Log de Consumo**: N/A (Não Aplicável). Nota: Este microsserviço atua apenas como Produtor.

Para ver os logs no Kubernetes: `kubectl logs -l app=users-api -f`

* * * * *

📂 **Estrutura de Pastas (Kubernetes)**

Os arquivos de manifesto estão localizados na pasta **/k8s**:
